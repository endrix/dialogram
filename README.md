...
`supportsElementCreation: false` is for a diagram that is a projection of a
source artifact: the graph is generated by reading a file, so a creation tool
has nowhere to write. It empties the palette — the platform's own categories
and the profile's `storageOptions.entityPaletteItems` alike. It says nothing
about edits that DO have somewhere to go: moves, routing and the persisted
layout are unaffected, and so is `edits`.
